...
////////////////////////////////////////////////////////////////////////////
public class MstarGLSurfaceView extends BaseGLSurfaceView {
    // private static String TAG = "GL2JNIView";
    private final boolean DEBUG = false;
    private final static String TAG = "MstarGLsurfaceView";
    private int surfaceWidth = 0;
    private int surfaceHeight = 0;
    private boolean bOnSurfaceCreated = false;

    public MGameEngine gameEngine = null;
    public EGL10 mEgl = null;
    public ArrayList<Runnable> mMsEventQueue = new ArrayList<Runnable>();

    public MstarGLSurfaceView(Context context) {

	super(context);
	init(false, 0, 0);
	gameEngine = MGameEngine.getInstance();
    }
    ...
    public MstarGLSurfaceView(Context context, boolean translucent, int depth,
	    int stencil) {
	super(context);
	init(translucent, depth, stencil);
    }

    public MGameEngine getGameEngine() {
	return gameEngine;
    }
	...
	public void destroyContext(EGL10 egl, EGLDisplay display,
		EGLContext context) {
	    Log.w(TAG,
		    ">>>>>>>>>>>>>>destroyContext OpenGL ES 2.0 context<<<<<<<<<<<");
	    // FIXME: call jni cleanup()?
	    // BaseUIActivity.this.cleanup();
	    MstarGLSurfaceView.this.gameEngine.cleanup();
	...
	public void onDrawFrame(GL10 gl) {
	    gameEngine.render(surfaceWidth, surfaceHeight, true);
	}

	public void onSurfaceChanged(GL10 gl, int width, int height) {
	    Log.v(TAG, String.format(" onSurfalceChanged2 %d: %d\n", width,
		    height));
	    // TODO: remove BaseUIActivity
	    if (MstarGLSurfaceView.this.getContext() instanceof Mstar3DUIActivity) {
		Mstar3DUIActivity activityMstar3DUI = (Mstar3DUIActivity) MstarGLSurfaceView.this.getContext();
		if (activityMstar3DUI.Isrender) {
		    surfaceWidth = width;
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
	    ...
	    if (MstarGLSurfaceView.this.getContext() instanceof Mstar3DUIActivity) {
		Mstar3DUIActivity activityMstar3DUI = (Mstar3DUIActivity) MstarGLSurfaceView.this.getContext();
		gameEngine.init(activityMstar3DUI.blendFile, activityMstar3DUI.configFile,
			"/data/data/com.android.mlauncher/files");
