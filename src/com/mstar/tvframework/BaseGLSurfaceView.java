 ...
 * 
 */
public class BaseGLSurfaceView extends SurfaceView implements
	SurfaceHolder.Callback {
    private final static String TAG = "BaseGLSurfaceView";
    ...
    /**
     * /** Inform the view that the activity is started. Do nothing now
     */
    public void onStart() {
	// Log.i("BaseGLSurfaceView", "====================== on start");
    }

    /**
     * Inform the view that the activity is sStopped. Do nothing now
     */
    public void onStop() {
	// Log.i("BaseGLSurfaceView", "======================= on Stop");
	mGLThread.onPause();
    }
    ...
    public void onPause() {
	// mGLThread.onPause();
	// Log.i("BaseGLSurfaceView", "======================= on onPause");
	mGLThread.onFreeze();
    }
	...
	public void destroyContext(EGL10 egl, EGLDisplay display,
		...
		// android.opengl.EGLLogWrapper is not public in android.opengl;
	...
	private void guardedRun() throws InterruptedException {
	    ...
	    mHaveEglContext = false;
	    mHaveEglSurface = false;
	    mRequestFreezed = false;
	    mSurfaceFailedOnce = false;
	    mSurfaceFailedNeedWait = false;
	    mFreezed = false;
	    try {
		GL10 gl = null;
		...
		while (true) {
			...
			while (true) {
			    ...
			    if (mFreezed != mRequestFreezed) {
				mFreezed = mRequestFreezed;
				sGLThreadManager.notifyAll();
				if (LOG_PAUSE_RESUME) {
				    Log.i("GLThread", "mFreezed is now "
					    + mFreezed + " tid=" + getId());
				}
			    }
				...
				// If create egl surface failed,
				// we must wait for another notify.
				if (mHaveEglSurface && !mSurfaceFailedNeedWait) {
				    if (mSizeChanged) {
					sizeChanged = true;
					w = mWidth;
					h = mHeight;
				    ...
				    if (mSurfaceFailedNeedWait) {
					createEglSurface = true;
					mSurfaceFailedNeedWait = false;
					mWaitingForSurface = true;
					Log.i("GLThread",
						"wait here until create surface");
					sGLThreadManager.notifyAll();
				    }
				}
			    }
			...
			if (gl == null) {
			    ...
			    Log.w("GLThread", "gl == null");
			    synchronized (sGLThreadManager) {
				Log.w("GLThread", "gl == null wait for notify");
				mSurfaceFailedOnce = true;
				mHaveEglSurface = false;
				mSurfaceFailedNeedWait = true;
				mWaitingForSurface = true;
			    }
			    // break; //Old code will destroy this thread.
			} else {

			    synchronized (sGLThreadManager) {
				Log.w("GLThread", "gl != null notify ALL");
		    ...
		    if (sizeChanged && !createEglSurface) {
			if (LOG_RENDERER) {
		    ...
		    if (!mFreezed && !createEglSurface) {
			mRenderer.onDrawFrame(gl);

			if (!mEglHelper.swap()) {
			    if (LOG_SURFACE) {
				Log.i("GLThread", "egl context lost tid="
					+ getId());
			    }
			    lostEglContext = true;
			}
	...
	// Add freeze mode to resolve two APK overlapped issue
	public void onFreeze() {
	    synchronized (sGLThreadManager) {
		if (LOG_PAUSE_RESUME) {
		    Log.i("GLThreed", "Onfreeze tid" + getId());
		}
		mRequestFreezed = true;
		sGLThreadManager.notifyAll();
		while ((!mFreezed)) {
		    if (LOG_PAUSE_RESUME) {
			Log.i("Main thread", "onFreeze waiting for mFreezed.");
		    }
		    try {
			sGLThreadManager.wait();
	...
	public void onUnfreeze() {
	    synchronized (sGLThreadManager) {
		if (LOG_PAUSE_RESUME) {
		    Log.i("GLThread", "OnUnfreeze tid" + getId());
		}
		mRequestFreezed = false;
		sGLThreadManager.notifyAll();
		while ((mFreezed)) {
		    if (LOG_PAUSE_RESUME) {
			Log.i("Main thread", "onUnfreeze waiting for mFreezed.");
		    }
		    try {
			sGLThreadManager.wait();
		    } catch (InterruptedException ex) {
	...
	public void onPause() {
		...
		mRequestPaused = true;
		mRequestFreezed = false; // unfreezed when true Pause;
		sGLThreadManager.notifyAll();
		while (((!mExited) && (!mPaused)) || (mFreezed)) {
		    if (LOG_PAUSE_RESUME) {
	...
	public void onWindowResize(int w, int h) {
		...
		while (!mExited && !mPaused && !mRenderComplete && (!mFreezed)
	...
	private boolean mRequestFreezed;
	private boolean mFreezed;
	private boolean mSurfaceFailedOnce;
	private boolean mSurfaceFailedNeedWait;
	...
	private void checkGLESVersion() {
		...
		mGLESVersion = SystemProperties.getInt("ro.opengles.version",
