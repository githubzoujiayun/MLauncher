...
public class AgentManager {
	...
	public void build() {
		...
		try {
			...
			Log.v(TAG,
					"Parse xml to getAllAgetn encounter exception!"
							+ ex.getMessage());
		}
	}
	...
	public boolean handleEvent(MGameEvent e) {
		...
		Log.v(TAG, "!!!!!!!!!!!!!!!!!!!!! AgentManager handleEvent agentName="
				+ agentName);
		if (agentName!=null && agentMap.containsKey(agentName)) {
			...
			Log.v(TAG,
					"AgentManager handleEvent can not find agent "
							+ e.getAgentName());
		}
		return false;
	}
		...
		try {
			...
			if (activity != null) {
				if (activity instanceof Mstar3DUIActivity) {
					...
					String agentFileName = activity_3DUI.agentFile
							.substring(lastSepa_pos + 1);
	//				Log.v(TAG, "parseAgentXml() function agentFileName=" + agentFileName);
...
//								Log.v(TAG,"++++++parse entity element,entity obj is " + entity);
								if (agent != null) {
//									Log.v(TAG,"!!!!!!!!!!!!!!!Agent append entity");
			...
			Log.v(TAG,
					"Parse xml to getAllAgent encounter exception:"
							+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (inputStream != null)
