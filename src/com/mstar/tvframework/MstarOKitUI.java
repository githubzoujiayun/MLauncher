...
public class MstarOKitUI {
	...
	public boolean update(String agentName, String entityName) {
		if (LOG_ALL_FUNCTION_CALL) {
			Log.v(TAG, "MstarOKitUI.update agentName=" + agentName
		...
		if (bInitialized) {
			...
			MGameEvent event = new MGameEvent(agentName,
					MGameEvent.EventType.E_EVENT_UPDATE);
	...
	public void prepareVECapture() {
		MGameEngine.getInstance().createDynamicTexture(WIDTH_VE, HEIGHT_VE, 0,
				VETEXTURE_PIXELFORMAT, VETEXTURE_NAME, VETEXTURE_GROUP, 0, 0,
				false, 0, 0);
	...
	public void bindVECapture(String agentName, String entityName) {
		...
		MGameEngine.getInstance().replacewithDynamicTex("testApp",
				entity.getSceneName(), entityName, 0, 0, 0, 0);
	}
