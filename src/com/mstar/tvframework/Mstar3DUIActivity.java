...
public class Mstar3DUIActivity extends Activity {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (!ret) {
			...
			// LEFT_BTN
			int btnType = 0;
			ret = gameEngine.axisEvent(coordType, btnType, event.getAction(),
	...
	public boolean onGenericMotionEvent(MotionEvent event) {
		...
		if ((event.getSource() & InputDevice.SOURCE_MOUSE) != 0) {
			...
			case MotionEvent.ACTION_HOVER_MOVE:
				...
				gameEngine.axisEvent(coordType, btnType,
						MotionEvent.ACTION_HOVER_MOVE, event.getX(),
						event.getY(), event);
				return true;
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		boolean ret = gameEngine.keyEvent(event.getAction(),
				event.getUnicodeChar(), event.getKeyCode(), event);
	...
	public void callBackInitDone(String sceneName) {
		Log.v(TAG,
				"++++++++++++++++++++++++Mstar3DUIActivity:callBackInitDone invoked");
