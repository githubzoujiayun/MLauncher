...
public class MGameEngine {
	...
	public native boolean init_native(String blendFile, String configFile,
			String EtcPath);
	...
	public native boolean keyEvent_native(int action, int unicodeChar,
			int keyCode, KeyEvent event);

	public native boolean render_native(int drawWidth, int drawHeight,
			boolean forceRedraw);

	public native boolean axisEvent_native(int coordType, int btnType,
			int action, float x, float y, MotionEvent event);

	public native boolean runLuaScript_native(String Script);

	public native boolean setImage_native(Bitmap image, String appName,
			String sceneName, String objName, int subEntityID, int techID,
			int passID, int stateID);

	public native boolean setImage_native(Bitmap image, String appName,
			String sceneName, String objName, int subEntityID, int techID,
			int passID, int stateID, boolean formatFlag);

	public native String queryCurrentScene_native();

	public native void createDynamicTexture_native(int width, int height,
			int num_mips, String pixelFormat, String name, String group,
			int texType, int usage, boolean hwGammaCorrection, int fsaa,
			int fsaaHint);

	public native void delDynamicTexture_native();

	public native boolean replacewithDynamicTex_native(String appName,
			String sceneName, String objName, int subEntityID, int techID,
			int passID, int stateID);
		...
		synchronized (this) {
			mMsEventQueue.add(new Runnable() {
				public void run() {
					 ...
					 ", keyCode = " + keyCodeL + "!! ");
					m_bRet = keyEvent_native(actionL, unicodeCharL, keyCodeL,
							eventL);
				}
			});
		}
	...
	public boolean axisEvent(int coordType, int btnType, int action, float x,
		...
		synchronized (this) {
			mMsEventQueue.add(new Runnable() {
				public void run() {
					m_bRet = axisEvent_native(coordTypeL, btnTypeL, actionL,
							xL, yL, eventL);
				}
			});
		}
	...
	public boolean setImage(Bitmap image, String appName, String sceneName,
		...
		synchronized (this) {
			mMsEventQueue.add(new Runnable() {
				public void run() {
					m_bRet = setImage_native(imageL, appNameL, sceneNameL,
							objNameL, subEntityIDL, techIDL, passIDL, stateIDL);
				}
			});
		}
		return m_bRet;
	...
	public void createDynamicTexture(int width, int height, int num_mips,
			String pixelFormat, String name, String group, int texType,
			int usage, boolean hwGammaCorrection, int fsaa, int fsaaHint) {
		...
		synchronized (this) {
			mMsEventQueue.add(new Runnable() {
				public void run() {
					createDynamicTexture_native(widthL, heightL, num_mipsL,
							pixelFormatL, nameL, groupL, texTypeL, usageL,
							hwGammaCorrectionL, fsaaL, fsaaHintL);
				}
			});
		}
	}
	...
	public boolean replacewithDynamicTex(String appName, String sceneName,
			String objName, int subEntityID, int techID, int passID, int stateID) {
		final String appNameL = appName;
		final String sceneNameL = sceneName;
		final String objNameL = objName;
		...
		synchronized (this) {
			mMsEventQueue.add(new Runnable() {
				public void run() {
					replacewithDynamicTex_native(appNameL, sceneNameL,
							objNameL, subEntityIDL, techIDL, passIDL, stateIDL);
				}
			});
		}
