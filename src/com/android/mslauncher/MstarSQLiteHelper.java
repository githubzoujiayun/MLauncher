...
public class MstarSQLiteHelper extends SQLiteOpenHelper
{
	...
	public MstarSQLiteHelper(Context context, String name,
	...
	public void onCreate(SQLiteDatabase db)
	{
		db.execSQL("create table if not exists mstaritem("
				...
				+ "scene TEXT NOT NULL," + "agent TEXT NOT NULL,"
				+ "object TEXT NOT NULL," + "title TEXT NOT NULL,"
		...
		try
		{
			...
			while ((position = parser.next()) != XmlResourceParser.END_DOCUMENT)
			{
				...
				if (XML_TAG_BEGIN.equals(name))
				{
					...
					iteminfo.setScene(parser.getAttributeValue(null,
							COLUMN_SCENE));
					iteminfo.setAgent(parser.getAttributeValue(null,
							COLUMN_AGENT));
					iteminfo.setObject(parser.getAttributeValue(null,
							COLUMN_OBJECT));
					iteminfo.setTitle(parser.getAttributeValue(null,
							COLUMN_TITLE));
					iteminfo.setPackageName(parser.getAttributeValue(null,
							COLUMN_PACKAGENAME));
					iteminfo.setClassName(parser.getAttributeValue(null,
							COLUMN_CLASSNAME));
					iteminfo.setLevel(parser.getAttributeIntValue(INDEX_6,
							DEFAULT_VALUE_0));
					iteminfo.setPage(parser.getAttributeIntValue(INDEX_7,
							DEFAULT_VALUE_0));
					iteminfo.setIconIndex(parser.getAttributeIntValue(INDEX_8,
							DEFAULT_VALUE_0));
					insert(db, iteminfo);
				}
			}
