...
public class MstarDatabaseProvider extends ContentProvider{
	...
	@Override
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String arg4) {
		...
		switch (mUriMatcher.match(uri)) {
			case TYPE_ALL:
				break;
			case TYPE_ITEM:
				sqlQueryBuilder.appendWhere(DATABASE_ID + MARK_EQUAL + uri.getPathSegments().get(1));
				break;
			default: {
		...
		SQLiteDatabase sqlDatabase = mOpenHelper.getWritableDatabase();
		Cursor cursor = sqlQueryBuilder.query(sqlDatabase, projection,
				selection, selectionArgs, null, null, DEFAULT_SORT_ORDER);
